...
// the validator's: both read the same case table, substituting only their
// own host and token, so a divergence in either service's check order or
// error vocabulary fails both suites rather than silently drifting apart.
func TestIngressContractCases(t *testing.T) {
	...
	const validHost = "assistant-gateway:8081" // validator copy uses assistant-validator:8082
