...
const hunterInstructions = "Hunter Assistant tools. Nearly all of these tools ONLY read data; they never create, update, delete, run, or send anything. The two exceptions are create_whiterabbit_template and create_ansible_playbook, which CREATE a new Control Center template or playbook (never edit, update, delete, or run one), and only when Control Center write is enabled. Every created artifact is validated server-side and rejected if it uses a disallowed module/command or embeds a secret.\n\n" +
