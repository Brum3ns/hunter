...
# The machine credentials rotated here are generated by ops/assistant/bootstrap.sh
# into the assistant_secrets volume, not by ops/assistant/prepare_secrets.sh (which
# only prepares the operator's ./secrets directory for the two provider keys and
# generates nothing). Run the same generator twice into isolated directories to
# produce an "old" and a "new" credential set to rotate between.
ASSISTANT_SECRET_TARGET="$temporary_dir/old" "$script_dir/bootstrap.sh" >/dev/null
ASSISTANT_SECRET_TARGET="$temporary_dir/new" "$script_dir/bootstrap.sh" >/dev/null
old_dir="$temporary_dir/old"
new_dir="$temporary_dir/new"
...
for name in $rabbit_secret_names; do
  ...
  [ "$(stat -c '%a' "$old_file")" = "400" ] || fail "$name old source mode"
  [ "$(stat -c '%a' "$new_file")" = "400" ] || fail "$name new source mode"
