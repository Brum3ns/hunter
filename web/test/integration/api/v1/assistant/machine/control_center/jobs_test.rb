...
class Api::V1::Assistant::Machine::ControlCenter::JobsTest < ActionDispatch::IntegrationTest
  ...
  test "get_job returns the full projection, excluding internal targeting fields" do
    record = job(
      template_name: "probe", status: "succeeded", queue_name: "test", target_count: 3,
      exit_status: 0, stdout: "ok", stderr: "",
    ...
    expected_keys = %w[id template_name status queue_name target_count exit_status created_at stdout stderr updated_at]
  ...
  def job(template_name:, status: "queued", queue_name: "test", target_count: 0, exit_status: nil,
           ...
           manual_targets: [], idempotency_key: nil, created_by: nil)
