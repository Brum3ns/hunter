...
class Cves::NormalizerTest < ActiveSupport::TestCase
  ...
  test "gathers nested cwe ids from a symbol-keyed database_specific" do
    symbol_keyed = RAW.merge(
      "affected" => [ {
        "package" => { "ecosystem" => "npm", "name" => "foo", "purl" => "pkg:npm/foo" },
        "versions" => [ "1.0.0" ],
        database_specific: { "cwe_ids" => [ "CWE-999" ] },
        "ranges" => []
      } ]
    )
    doc = Cves::Normalizer.call(symbol_keyed)
    assert_includes doc["cwe_ids"], "CWE-999"
  end

  test "collapses a cwe id duplicated between top-level and nested database_specific" do
    duplicated = RAW.merge(
      "database_specific" => { "cwe_ids" => [ "CWE-80" ] },
      "affected" => [ {
        "package" => { "ecosystem" => "npm", "name" => "foo", "purl" => "pkg:npm/foo" },
        "versions" => [ "1.0.0" ],
    ...
    doc = Cves::Normalizer.call(duplicated)
    assert_equal [ "CWE-80" ], doc["cwe_ids"]
  end
